...
fun DatabaseInterface.createMotedeltakerArbeidsgiver(
    ...
    this.connection.use { connection ->
        ...
        val motedeltakerArbeidsgiverIdList = connection.prepareStatement(queryCreateMotedeltakerArbeidsgiver).use {
            ...
            it.setTimestamp(3, now)
            it.setInt(4, moteId)
            it.setString(5, dialogmotedeltakerArbeidsgiver.virksomhetsnummer.value)
            it.setString(6, dialogmotedeltakerArbeidsgiver.lederNavn)
            it.setString(7, dialogmotedeltakerArbeidsgiver.lederEpost)
            it.executeQuery().toList { getInt("id") }
        }
